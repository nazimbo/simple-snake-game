...
- **`audio.js`** — `AudioManager`: procedural chiptune music + SFX via the Web Audio API (no audio files). The `AudioContext` is created lazily on the first user gesture (browsers block autoplay), so `resume()`/`startMusic()` are called from `startGame`/`restart`/unpause. Music is a 4-bar loop (Am–F–C–G) played by a **look-ahead scheduler** (`setInterval` tick queues notes ahead on the audio clock for tight timing) over `LEAD`/`BASS`/`ARP` MIDI pattern arrays plus drums (kick + noise-buffer snare/hat), routed through a low-pass filter and an echo send. SFX (`playEat`, `playGameOver`, `playWin`) are short enveloped oscillators. Mute state persists in `localStorage` under `snakeMuted`. `Game` owns an instance as `this.audio` and every call is null-guarded (Web Audio may be absent).
