...
- **`audio.js`** — `AudioManager`: procedural chiptune music + SFX via the Web Audio API (no audio files). The `AudioContext` is created lazily on the first user gesture (browsers block autoplay), so `resume()`/`startMusic()` are called from `startGame`/`restart`/unpause. Tracks are plain data in the `MUSIC_TRACKS` array (each a 4-bar loop of `lead`/`bass`/`arp` MIDI patterns + `kick`/`snare`/`hat` drums + `bpm`/`leadType`; `silent: true` is the "Off" track) — adding one is just another entry. A **look-ahead scheduler** (`setInterval` tick queues notes ahead on the audio clock for tight timing) plays the selected track through a low-pass filter and an echo send. The chosen track persists in `localStorage` under `snakeTrack`; `setTrack` hot-swaps it. SFX (`playEat`, `playGameOver`, `playWin`) are short enveloped oscillators. Mute state persists under `snakeMuted`. `Game` owns an instance as `this.audio` and every call is null-guarded (Web Audio may be absent); the start screen exposes a track picker (`setupMusicPicker`) alongside the theme picker.
